...
def insert_row(insert_dict,test=True):
	...
	conn = _get_db_conn()
	cursor = conn.cursor()
	cursor.execute("insert into alert_on_change(command, common_threshold, email_address, description, cadence, ignore_output, output, follow_on_command,status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(command,common_threshold,email_address,description,cadence,ignore_output.encode('latin-1'),output.encode('latin-1'),follow_on_command,status))
	if not test:
		conn.commit()
	cursor.close()
...
def send(test=True):
	...
	# execute our Query
	cursor.execute("select alert_on_change_id, command, output, common_threshold, email_address, description, last_updated, cadence, ignore_output, ok_exit_codes, follow_on_command, random_key from alert_on_change where status = 'A'")
	...
	for row in cursor:
		...
		if False and int(time.time()) - int(last_updated) < cadence:
			...
			continue
		(status,new_output) = commands.getstatusoutput("""timeout 10 /bin/bash -c '""" + command + """'""")
		...
		if not test and common_percent < int(common_threshold):
				...
				if follow_on_command != '':
					...
					follow_on_output = ''
				message = 'Output of command described as:\n\n' + description + '''\n\nhas changed.\n\nCOMMAND:\n\n''' + command
				if follow_on_output != '':
					message += '''\n\nFOLLOW-ON-OUTPUT\n\n''' + follow_on_output
				message += '''\n\nWORD DIFF:\n\n''' + dwdiff_output + '''\n\nDIFF:\n\n''' + diff
				mailgun.Mailgun.init("MAILGUNAPIUSER")
				mailgun.MailgunMessage.send_txt("MAILGUNADDRESS",email_address,'Alert on change triggered!',message)
